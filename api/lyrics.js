...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (rawSong) {
      ...
      songHit = songHitsOnly.find(hit => {
        const title = `${hit.result.primary_artist.name} ${hit.result.title}`.toLowerCase();
        return title.includes(cleanedLower);
      });
    }
    ...
    const match = songs.find(song =>
      song.title.toLowerCase().includes(rawSong || cleanedTitle.toLowerCase())
    );
